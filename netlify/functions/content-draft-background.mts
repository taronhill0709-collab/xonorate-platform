...
async function handler(req: Request) {
  ...
  const { jobId, input } = (await req.json()) as { jobId?: string; input?: ContentDraftInput };
  if (!jobId || !input) {
  ...
  try {
    const body = await draftContentBody(input);
    await setContentDraftJobStatus(jobId, { status: "done", body });
