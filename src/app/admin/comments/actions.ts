...
export async function toggleSignatureCommentPin(signatureId: string, pinned: boolean) {
  await requireAdmin();
  ...
  await db.update(signatures).set({ commentPinned: pinned }).where(eq(signatures.id, signatureId));
