...
export async function setInquiryStatus(
  ...
  revalidatePath(`/admin/case-submissions/${inquiryId}`);
  redirect(`/admin/case-submissions/${inquiryId}?saved=1`);
}
...
export async function requestMoreInfo(inquiryId: string, formData: FormData) {
  ...
  if (parsed.items.length === 0 && !note) {
    redirect(`/admin/case-submissions/${inquiryId}?error=select-at-least-one`);
  }
  ...
  const [inquiry] = await db
    ...
    .limit(1);
  if (!inquiry) redirect("/admin/case-submissions");
...
export async function deleteInfoRequest(inquiryId: string, requestId: string) {
  await requireAdmin();
  await db.delete(inquiryInfoRequests).where(eq(inquiryInfoRequests.id, requestId));
  revalidatePath(`/admin/case-submissions/${inquiryId}`);
  redirect(`/admin/case-submissions/${inquiryId}?saved=deleted`);
}
