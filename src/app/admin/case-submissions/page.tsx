...
import { setCaseSubmissionStatus } from "./actions";
...
export default async function AdminCaseSubmissionsPage() {
  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        <p className="mt-6 text-sm text-muted">No case submissions yet.</p>
      ) : (
          ...
          {rows.map((row) => (
            <div key={row.id} className="rounded-lg border border-border p-4 text-sm">
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                {row.status === "new" && (
                  <form action={setCaseSubmissionStatus.bind(null, row.id, "reviewing")}>
                ...
                {row.status === "reviewing" && (
                  <form action={setCaseSubmissionStatus.bind(null, row.id, "accepted")}>
                    <button type="submit" className="text-brand underline">
                      Accept
                    </button>
                  </form>
                ...
                {(row.status === "new" || row.status === "reviewing") && (
                  <form action={setCaseSubmissionStatus.bind(null, row.id, "declined")}>
                ...
                {row.status === "accepted" && (
                  ...
                  >
                    Create case from this submission
                  </Link>
                )}
              </div>
            </div>
