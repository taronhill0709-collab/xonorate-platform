...
import { SUPPORT_LETTER_PURPOSE_LABELS, SUPPORT_LETTER_STATUS_LABELS, type SupportLetterPurpose } from "@/family/support-letters-types";
...
export default async function SupportLettersPage({
  ...
  const letters = await listSupportLettersForFamily(familyId);
      ...
      {letters.length === 0 ? (
        <section className="rounded-2xl border border-dashed border-border bg-muted-background p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm text-muted">
            Answer a few guided questions in your own words, and we&rsquo;ll help you turn them into
          ...
          {letters.map((letter) => {
            ...
            return (
              <li key={letter.id} className="rounded-2xl border border-border bg-muted-background p-5">
                ...
                {content && (
                  <p className="mt-3 line-clamp-2 text-sm text-muted">{content}</p>
                )}
              </li>
            );
          })}
