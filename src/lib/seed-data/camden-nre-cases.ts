// Shared source data for the three Camden, NJ NRE-sourced exonerated-person
// spotlight cases (Sean Washington, Kevin Baker, Anthony Ways) — used by both
// scripts/seed-camden-nre-cases.ts (local dev) and the temporary
// src/app/api/admin/seed-camden-cases/ route (production), so the two never
// drift out of sync. See scripts/seed-camden-nre-cases.ts for sourcing notes.
import type { cases } from "@/db/schema";

export const CAMDEN_NRE_CASES: (typeof cases.$inferInsert)[] = [
    {
